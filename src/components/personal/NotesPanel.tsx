...
import MeetingNotesEditor, { richTextToPlainText } from "./MeetingNotesEditor";
...
export default function NotesPanel({ userId, initialNoteId, onInitialNoteOpened, onCreateTodo }: { userId: string | null; initialNoteId?: string | null; onInitialNoteOpened?: () => void; onCreateTodo: (origin: { id: string; label: string; type: "note" }) => void }) {
  ...
  const visible = useMemo(() => { const term = search.trim().toLocaleLowerCase("pt-BR"); return items.filter((item) => !term || `${item.title} ${richTextToPlainText(item.content)}`.toLocaleLowerCase("pt-BR").includes(term)); }, [items, search]);
  ...
  return <section className={styles.workspacePanel} aria-labelledby="notes-title"><header className={styles.workspaceHeader}><div><span>ARQUIVO PESSOAL</span><h2 id="notes-title">Notas</h2></div><button className={styles.primaryButton} type="button" onClick={startCreate}><Plus size={15} /> Nova nota</button></header><div className={styles.notesToolbar}><label className={styles.workspaceSearch}><Search size={14} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar notas..." /></label></div>{error && <p className={styles.error}>{error}</p>}<div className={styles.notesGrid}>{loading ? <div className={styles.empty}>Carregando notas...</div> : visible.length === 0 ? <div className={styles.empty}><FileText size={22} /><strong>Nenhuma nota encontrada</strong><span>Guarde ideias e lembretes pessoais.</span></div> : visible.map((note) => <article key={note.id} className={styles.noteCard} role="button" tabIndex={0} onClick={() => open(note)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") open(note); }}><h3>{note.title}</h3><p>{richTextToPlainText(note.content) || "Nota sem conteúdo."}</p><time>{relativeTime(note.updated_at)}</time></article>)}</div>{(creating || current) && <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><form className={styles.documentModal} onSubmit={save}><header><div><span>{creating ? "NOVA NOTA" : "NOTA"}</span><h2>{creating ? "Criar nota" : current?.title}</h2></div><button type="button" aria-label="Fechar" onClick={close}><X size={18} /></button></header><div className={styles.noteEditor}><label><span>Título</span><input required maxLength={180} value={title} onChange={(event) => setTitle(event.target.value)} /></label><div><span className={styles.editorFieldLabel}>Conteúdo</span><MeetingNotesEditor value={content} onChange={setContent} ariaLabel="Conteúdo da nota" placeholder="Escreva sua nota ou digite / para inserir um bloco..." /></div></div>{error && <p className={styles.modalError}>{error}</p>}<footer>{current && <><button type="button" className={styles.dangerButton} onClick={() => void remove(current)}><Trash2 size={14} /> Excluir nota</button><button type="button" className={styles.secondaryButton} onClick={() => onCreateTodo({ id: current.id, label: current.title, type: "note" })}><Plus size={14} /> Criar ToDo</button></>}<span /><button type="button" className={styles.secondaryButton} onClick={close}>Cancelar</button><button type="submit" className={styles.primaryButton} disabled={saving}>{saving ? "Salvando..." : "Salvar"}</button></footer></form></div>}</section>;
}
